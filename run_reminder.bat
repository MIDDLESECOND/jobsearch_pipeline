...
set "REMINDER_MSG=%~1"
if /I "%~1"=="bayarea" set "REMINDER_MSG=Corpus-mode check-in (jobsearch_pipeline): about a month in the Bay Area now. 1) Re-point the config.yaml searches to Bay Area + remote. 2) Decide whether Adzuna comes back (dropped 2026-09-09, see CHANGELOG). 3) Confirm the corpus is still growing: python pipeline.py stats"
