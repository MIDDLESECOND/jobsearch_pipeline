...
def test_age_full_timestamp_hours():
    assert report.posting_age("2026-07-04T06:00:00", "2026-07-04T08:00:00", now=NOW) == "3h ago"
...
def test_age_months():
    assert report.posting_age("2026-01-04T09:00:00", "2026-07-04T08:00:00", now=NOW) == "6mo ago"


def test_age_under_one_hour_is_just_now():
    assert report.posting_age("2026-07-04T08:30:00", "2026-07-04T08:31:00", now=NOW) == "just now"
...
def test_age_date_only_today_falls_back_to_seen_hedged():
    ...
    assert report.posting_age("2026-07-04", "2026-07-04T06:00:00", now=NOW) == "seen 3h ago"
...
def test_age_date_only_day_ahead_of_seen_is_hedged_not_future():
    ...
    assert report.posting_age("2026-07-05", "2026-07-04T21:05:00",
...
def test_age_date_only_past_stays_day_granularity():
    ...
    assert report.posting_age("2026-07-02", "2026-07-04T08:00:00", now=NOW) == "2d ago"


def test_age_date_only_ats_backlog_months_old():
    assert report.posting_age("2026-04-01", "2026-07-04T08:00:00", now=NOW) == "3mo ago"


def test_age_empty_falls_back_to_seen():
    assert report.posting_age("", "2026-07-04T06:00:00", now=NOW) == "seen 3h ago"


def test_age_garbage_falls_back_to_seen():
    assert report.posting_age("not-a-date", "2026-07-04T06:00:00", now=NOW) == "seen 3h ago"
...
def test_age_adzuna_utc_z_suffix_parses():
    ...
    label = report.posting_age("2026-07-03T07:00:00Z", "2026-07-04T08:00:00", now=NOW)
    assert label.endswith("ago") and not label.startswith("seen")


def test_age_future_date_clamps_to_just_now():
    assert report.posting_age("2026-07-04T11:00:00", "2026-07-04T08:00:00", now=NOW) == "just now"


def test_age_nothing_usable_is_empty():
    assert report.posting_age("", "", now=NOW) == ""
...
def test_above_line_fresh_beats_above_line_stronger_but_older():
    ...
    ordered = sorted([old_strong, fresh_ok], key=report.recency_sort_key)
...
def test_below_line_never_outranks_above_line():
    ...
    ordered = sorted([fresh_weak, old_ok], key=report.recency_sort_key)
...
def test_below_line_ordered_by_fit_then_recency():
    ...
    ordered = sorted([weaker_fresh, weak_old, weak_fresh], key=report.recency_sort_key)
...
def test_ats_backlog_ranks_by_posted_date_not_seen_date():
    ...
    ordered = sorted([backlog, yesterday], key=report.recency_sort_key)
...
def test_no_timestamps_sorts_last_in_band():
    ...
    undated = _row(12, "", "")
    ordered = sorted([undated, dated], key=report.recency_sort_key)
...
def test_far_future_date_neither_crashes_nor_pins_top():
    ...
    ordered = sorted([day_only, full_ts, aware, fresh], key=report.recency_sort_key)
    assert ordered[0] is fresh
    # And the label hedges rather than claiming the garbage date as real:
    assert report.posting_age("9999-12-31", "2026-07-04T08:00:00", now=NOW) == "seen 1h ago"
...
def test_pre_2000_date_treated_as_garbage():
    ...
    key = report.recency_sort_key(_row(12, "1970-01-01T09:00:00", "2026-07-04T08:00:00"))
    assert key[0] == 0  # above the line, no crash
    assert report.posting_age("1970-01-01T09:00:00", "2026-07-04T08:00:00", now=NOW) == "seen 1h ago"
...
def test_day_ahead_date_sorts_by_first_seen_not_future_midnight():
    ...
    assert report.recency_sort_key(utc_ahead) == report.recency_sort_key(no_date)


def test_none_fit_score_treated_as_zero():
    key = report.recency_sort_key(_row(None, "", ""))
...
def test_report_malformed_date_fails_fast(tmp_path, conn):
    ...
    with pytest.raises(ValueError):
        report.generate_report(cfg, conn, for_date="2026-7-4")
