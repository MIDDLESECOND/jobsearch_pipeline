...
Tests run against a real SQLite schema (pipeline.get_db builds it) but in a throwaway
...
def make_job(conn, *, job_url=None, title="Data Analyst", company="Acme Corp",
    ...
    norm_company = pipeline._norm_company(company)
    if norm_title is None:
        norm_title = pipeline._norm_title(title)
